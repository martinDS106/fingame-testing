...
export function CoinsCounter({
  ...
  const rtl = useRTL();
  const handlePress = onPress ?? (() => router.push('/marketplace-home' as never));

  return (
    <Pressable
      onPress={handlePress}
      className={`bg-accent-400 active:bg-accent-500 px-3 py-1.5 rounded-full ${className}`}
      style={[
        {
          alignItems: 'center',
          gap: 6,
